...
namespace CurveFit {
    public class Minimizer {
        ...
        public static int INITIALIZATION_FAILURE = 1;
        ...
        private static double C_REFLECTION = 1.0;  // reflection coefficient
        private static double C_CONTRACTION = 0.5;  // contraction coefficient
        private static double C_EXPANSION = 2.0;  // expansion coefficient
        private static double C_SHRINK = 0.5;  // shrink coefficient
        private static int ITER_FACTOR = 750;  // maximum number of iterations per numParams^2; twice that value for 2 threads
        private static int WORST = 0, NEXT_WORST = 1, BEST = 2;//indices in array to pass the numbers of the respective vertices
        ...
        private bool useSingleThread = true;
        private int status;                     // SUCCESS or error code
        private bool wasInitialized;         // initialization was successful at least once
        private double[] result;                // result data+function value
        private List<double[]> resultsVector; // holds several results if multiple tries; the best one is kept.
        private String ijStatusString = null;   // shown together with iteration count in status, no display if null
        private bool checkEscape;            // whether to stop when Escape is pressed
        ...
        private long startTime;                 // of the whole minimization process
                                                /*private Hashtable<Thread, List<double[]>> simpTable =
                                                        new Hashtable<Thread, List<double[]>>(); //for each thread, holds a reference to its simplex */
        ...
        public int minimize(double[] initialParams, double[] initialParamVariations) {
            ...
            for (int i = 0; i < maxLoopCount; i++) {        // try several times, until we have twice the same result
                ...
                if (maxRestarts > 0 && !useSingleThread) {  // set up 2nd thread to minimize
                    int seed = randomSeed + 1000000 + i;
                    Thread thread = new Thread(e => minimizeOnce(initialParams, initialParamVariations, seed));
                    thread.Start();
                    secondThread = thread;
                }
                ...
                if (resultsVector.Count == 0 && result == null)
                    return status;
                if (result == null)
                    result = (double[])resultsVector[0];
                foreach (var r in resultsVector)        // find best result so far
                ...
                for (int ir = 0; ir < resultsVector.Count; ir++)
                    if (!belowErrorLimit(value((double[])resultsVector[ir]), value(result), 1.0)) {
                        resultsVector.RemoveAt(ir);       // discard results that are significantly worse
                        ir--;
                    }
                if (resultsVector.Count >= 2) return SUCCESS;  // if we have two (almost) equal results, it's enough
            } //for i <= maxRestarts
        ...
        private void shrinkSimplexAndEvaluate(List<double[]> simp, int best) {
            for (int v = 0; v < numVertices; v++)
                if (v != best) {
                    for (int i = 0; i < numParams; i++)
        ...
        private bool belowErrorLimit(double highest, double lowest, double sensitivity) {
        ...
        private bool belowResolutionLimit(List<double[]> simp, int best) {
            for (int v = 0; v < numVertices; v++)
        ...
        private bool belowResolutionLimit(double[] vertex1, double[] vertex2) {
            for (int i = 0; i < numParams; i++)
        ...
        private void findValidInitalParams(double[] params1, double[] initialParamVariations, Random random) {
            int maxAttempts = 50 * numParams * numParams;  //max number of attempts to find params that do not lead to NaN
            double rangeFactor = 1;             // will gradually become larger to handle different orders of magnitude
            double rangeMultiplyLog = Math.Log(1e20) / (maxAttempts - 1); //will try up to 1e-20 to 1e20*initialParamVariations
            ...
            for (int i = 0; i < numParams; i++) {
                firstParams[i] = double.IsNaN(params1[i]) ? 0 : params1[i];
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++) {
                for (int i = 0; i < numParams; i++) {
                    ...
                    double multiplier = attempt < (maxAttempts / 10)
                        ? 1
                        : Math.Exp(rangeMultiplyLog * attempt * 2 * (random.NextDouble() - 0.5));
                    params1[i] = multiplier * (firstParams[i] + 2 * (random.NextDouble() - 0.5) * variations[i]);
                }
                evaluate(params1);
                //showVertex(params,"findValidInitalParams attempt "+attempt);
                if (!double.IsNaN(value(params1))) return;    //found a valid parameter set
            }
        }
        ...
        private bool reInitializeSimplex(List<double[]> simp, int bestVertexNumber, double[] paramVariations, Random random) {
         ...
         *  Returns false on error (if it could not create a simplex not resulting in NaN).
         */
        private bool initializeSimplex(List<double[]> simp, double[] paramVariations, Random random) {
            ...
            int maxAttempts = 100 * numParams;  //max number of attempts to find params that do not lead to NaN
            for (int v = 1; v < numVertices; v++) {
                ...
                do {                                //try finding a vertex that does not yield NaN
                    ...
                    for (int i = 0; i < numParams; i++)
                        simp[v][i] = variations[i] * (random.NextDouble() - 0.5);
                    ...
                    double nonZeroRandom = -1 + 1.8 * random.NextDouble();
         ...
         */
        private double[] makeNewParamVariations(List<double[]> simp, int bestVertexNumber,
            ...
            double WORST_RATIO = 1e-3;        //parameter variation should not be lower than typical value by more than this
            for (int i = 0; i < numParams; i++)
        ...
        private void evaluate(double[] vertex) {
            vertex[numParams] = userFunction.userFunction2(vertex, 0);
        }
        ...
        void copyVertex(double[] newVertex, double[] vertex) {
            Array.Copy(newVertex, 0, vertex, 0, newVertex.Length);
        }

        /** Find the simplex vertices with the worst, nextWorst and best values
         *  of the target function */
        private void order(List<double[]> simp, int[] worstNextBestArray) {
        ...
        private void showSimplex(List<double[]> simp, String heading) {
            lock (locker) {
                for (int i = 0; i < numVertices; i++)
        ...
        private void showVertex(double[] vertex, String heading) {
            lock (locker) {
                var s = "";
